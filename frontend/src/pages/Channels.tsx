...
const EMPTY_FORM = { name: '', type: 'whatsapp', phoneNumber: '', phoneNumberId: '', wabaId: '', accessToken: '' };
...
export default function Channels() {
  ...
  const openEdit = (ch: Channel) => {
    setEditing(ch);
    setForm({ name: ch.name, type: ch.type, phoneNumber: ch.phoneNumber || '', phoneNumberId: ch.phoneNumberId || '', wabaId: ch.wabaId || '', accessToken: ch.accessToken || '' });
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        const { data } = await channelsApi.update(currentWorkspace.id, editing.id, form);
        updateChannel(data);
        if (selected?.id === editing.id) setSelected(data);
      } else {
        const { data } = await channelsApi.create(currentWorkspace.id, form);
        addChannel(data);
        setSelected(data);
      }
      setShowForm(false);
    ...
    <AppLayout>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-gray-500 capitalize">{ch.type}</span>
                        </div>
                      </div>
            ...
            <div className="p-5 border-b border-gray-100">
              <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                  ...
                  <span className="text-sm text-gray-500 capitalize">{selected.type}</span>
                </div>
              </div>
            </div>
      ...
      >
        <div className="space-y-4">
          <Input label="Channel name *" placeholder="WhatsApp Business" value={form.name} onChange={f('name')} autoFocus />
          <div>
